...
webserver.get('/db', async (response) => {
    ...
    try {
        ...
        const [rows] = await mysqlPool.execute('SELECT * FROM world WHERE id = ? LIMIT 1', [random]);
...
webserver.get('/queries', async (response, request) => {
    ...
    try {
        ...
        for (let i = 0; i < queriesCount; i++) {
            const random = Math.floor(Math.random() * 9999) + 1;
            databaseJobs.push(mysqlPool.execute('SELECT * FROM world WHERE id = ? LIMIT 1', [random]));
        }
